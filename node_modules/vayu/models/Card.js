...
{
    this.Model.find({u: userId, b: boardId, a: null}, function (err, doc) {
        if (err) {
            fail(err);
        } else {
            success(doc);
...
p.updateCard = function(userId, id, lid, name, vtodos, success, fail) {
    this.Model.update({
        u: userId,
        _id: id
    }, {
...
p.archiveCard = function(userId, id, success, fail) {
    this.Model.update({
        u: userId,
        _id: id
    }, {
...
card.init('Card', 'card', {
    ...
    vtodos: [],
    //archived flag, set it to null if you want to un-archive it and display in the UI
    a: Boolean,
    //created date
